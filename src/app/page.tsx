...
export default function RentalInsightsPage() {
  ...
  const structuralOptions = PRICING_DATA.find((cat) => cat.id === "structural")!
  ...
  const handleStructuralFeatureChange = (
    featureId: string,
    checked: boolean
  ) => {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await getRentalPrediction(
        imagePreviews,
        structuralFeatures
      );
      if (result.error) {
        ...
        toast({
          variant: "destructive",
          title: "Analysis Failed",
          description: "No prediction data returned.",
        });
        setStep("confirm");
      }
    });
  ...
  const renderUploadStep = () => (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Upload 1 to {MAX_FILES} images of the room. Clear photos from
      ...
      <CardContent>
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            PNG, JPG, or WEBP (max {MAX_FILE_SIZE / 1024 / 1024}MB each)
          </p>
          <input
            id="file-upload"
            type="file"
  ...
  const renderConfirmStep = () => (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Verify the images and provide additional details about the property.
        </CardDescription>
      </CardHeader>
    ...
    <Card className="w-full max-w-md text-center">
      ...
      <CardContent>
        <p className="text-muted-foreground">
          Our AI is examining your images to identify features and calculate the
          perfect rental price. This might take a moment.
        </p>
      ...
      <Card className="w-full max-w-2xl">
        <CardHeader>
          ...
          <CardDescription className="text-center text-5xl font-bold text-foreground py-4">
            ₹{result.rentRange.lower.toLocaleString()} - ₹
            {result.rentRange.upper.toLocaleString()}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <Separator />
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-green-500" /> Detected &
              Confirmed Features
            </h3>
        ...
        {step === "upload" && !heroImage && (
          <div className="text-center mb-8 max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-bold font-headline">
              AI-Powered Rental Insights
            </h1>
            <p className="mt-4 text-lg text-muted-foreground">
              Get an instant, data-driven rent estimate for any room. Just
